...
def classificatoin(X_test):
    ...
    classification_result_df = pd.DataFrame(columns = ['id', 'job_title', 'task_type'])
    classification_result_df['id'] = X_test['id']
    classification_result_df['job_title'] = oe.inverse_transform(y_preds_processed.reshape(-1, 1)).ravel()
